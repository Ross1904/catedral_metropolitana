...
try {
    $stmt = $pdo->prepare("SELECT * FROM documentos_actas WHERE id = :id AND tipo_documento = 'Confirmacion'");
    $stmt->bindParam(':id', $id_documento);
    $stmt->execute();
    $acta = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$acta) {
        die("Error: El documento no existe o no es de Confirmación.");
    }
    ...
    $ciudad_nac = isset($datos['ciudad_nacimiento']) ? htmlspecialchars($datos['ciudad_nacimiento']) : '';
    $estado_nac = isset($datos['estado_nacimiento']) ? htmlspecialchars($datos['estado_nacimiento']) : '';
    $notas_marginales = isset($datos['notas_marginales']) && !empty($datos['notas_marginales']) ? htmlspecialchars($datos['notas_marginales']) : '';
    ...
    <head>
        <meta charset="UTF-8">
        <title>Certificado de Confirmación - <?php echo htmlspecialchars($acta['nombre_principal']); ?></title>
        <style>
    ...
    <body>
        ...
        <div class="centro">
            ...
            <h1 style="text-decoration: underline;">CERTIFICADO DE CONFIRMACION</h1>
        ...
        <div class="datos-libro">
            Libro: <span class="linea"><?php echo htmlspecialchars($acta['libro']); ?></span> &nbsp;
        ...
        <div class="cuerpo-exterior">
            Se encuentra asentada una Partida de CONFIRMACION que dice:<br>
            ...
            Fue Confirmado(a) <span class="linea negrita" style="min-width: 450px; text-transform: uppercase;"><?php echo htmlspecialchars($acta['nombre_principal']); ?></span><br>
            
            Hijo(a) de <span class="linea" style="min-width: 250px;"><?php echo htmlspecialchars($datos['nombre_padre'] ?? ''); ?></span> 
            y de <span class="linea" style="min-width: 250px;"><?php echo htmlspecialchars($datos['nombre_madre'] ?? ''); ?></span><br>
            
            Que nació en <span class="linea" style="min-width: 250px;"><?php echo $ciudad_nac; ?></span> 
            Estado: <span class="linea" style="min-width: 150px;"><?php echo $estado_nac; ?></span><br>
            
            El día <span class="linea" style="min-width: 40px; text-align:center;"><?php echo $dia_nac; ?></span> 
            del mes de <span class="linea" style="min-width: 120px; text-align:center;"><?php echo $mes_nac; ?></span> 
            del año: <span class="linea" style="min-width: 60px; text-align:center;"><?php echo $ano_nac; ?></span><br>

            Padrino: <span class="linea" style="min-width: 300px;"><?php echo htmlspecialchars($datos['nombre_padrino'] ?? ''); ?></span><br>
            Madrina: <span class="linea" style="min-width: 300px;"><?php echo htmlspecialchars($datos['nombre_madrina'] ?? ''); ?></span><br>
            Ministro: <span class="linea" style="min-width: 300px;"><?php echo htmlspecialchars($datos['ministro'] ?? ''); ?></span>
        </div>
